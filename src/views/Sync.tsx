  ...
  return (
    <div className="section-stack media-wide sync-page">
      <div className="cat-header">
        ...
        <span className="cat-sub">{trackCount} track{trackCount === 1 ? "" : "s"} in your library</span>
      ...
      <div className="settings-group">
        <p className="field-hint">
          Copy your music onto a connected MP3 player or SD card, organized into{" "}
          <code>{cleanFolder}/Artist/Album</code>. Re-syncing only copies what&apos;s new, so it&apos;s quick.
        </p>
        ...
        <div className="sync-controls">
          ...
          <label className="sync-mirror" title="Also delete tracks on the device that are no longer in your Library">
            <input type="checkbox" checked={mirror} onChange={(e) => setMirror(e.currentTarget.checked)} disabled={syncingMount !== null} />
            <span>Mirror (remove extras)</span>
          </label>
        </div>
        {mirror && (
          <p className="field-hint sync-warn">
            <Icon icon={triangleAlert} size="sm" /> Mirror deletes audio in <code>{cleanFolder}/</code> on the device that isn&apos;t in your Library. Other files are left alone.
          </p>
        )}

        <div className="sync-controls">
          <label className="sync-mirror" title="Also write your playlists into a Playlists/ folder at the device root">
            <input type="checkbox" checked={syncPlaylists} onChange={(e) => onPlaylistsChange(e.currentTarget.checked)} disabled={syncingMount !== null} />
            <span>Create playlists</span>
          </label>
          {syncPlaylists && (
            <div className="seg sync-mode" role="group" aria-label="Playlist format">
              <button type="button" className={playlistMode === "m3u8" ? "active" : ""} disabled={syncingMount !== null} onClick={() => onPlaylistModeChange("m3u8")}>.m3u8 files</button>
              <button type="button" className={playlistMode === "folders" ? "active" : ""} disabled={syncingMount !== null} onClick={() => onPlaylistModeChange("folders")}>Folders</button>
            </div>
          )}
        </div>
        {syncPlaylists && (
          <p className="field-hint">
            ...
            {playlistMode === "m3u8" ? (
              <>Writes <code>Playlists/&lt;name&gt;.m3u8</code> at the device root, each pointing at your <code>{cleanFolder}/</code> tracks with <code>../{cleanFolder}/…</code> paths — <b>no audio duplicated</b>. Best when your player reads .m3u files.</>
            ) : (
              <>Creates a folder per playlist under <code>Playlists/</code> with the tracks <b>copied</b> in (numbered to keep order) — for players that only do folder playlists. Note this <b>duplicates</b> those tracks on the card.</>
            )}
          </p>
        )}
      </div>
