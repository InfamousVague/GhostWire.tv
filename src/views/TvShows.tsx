...
import {
  removeFromLibrary, searchSources, tvEpisodes, tvSearch,
...
import { chevronLeft, circlePlay, download, flame, images, library, rotateCw, search as searchIcon, tv } from "../lib/icons";
...
export function TvShows({ onPlayLocal, onPlay, onAddToLibrary, posterFor, onReplacePoster }: TvShowsProps) {
  ...
  const shows = useMemo(() => {
    const map = new Map<string, { title: string; episodes: DownloadedItem[]; addedAt: number }>();
    for (const it of items) {
      const key = norm(it.title);
      const g = map.get(key);
      if (!g) {
        map.set(key, { title: it.title, episodes: [it], addedAt: it.addedAt });
      } else {
        g.episodes.push(it);
        g.addedAt = Math.max(g.addedAt, it.addedAt);
        if (/['’]/.test(it.title) && !/['’]/.test(g.title)) g.title = it.title;
      }
    }
  ...
  return (
    <div className="poster-card" onClick={onClick} onContextMenu={onContextMenu} role="button" tabIndex={0}>
      <div className="poster" style={poster ? undefined : { background: bg }}>
        {poster ? <img className="poster-img" src={poster} alt="" loading="lazy" /> : <span className="poster-glyph"><Icon icon={tv} size="2xl" /></span>}
  ...
  return (
    <div className="series media-wide">
      ...
      <div className="series-head">
        <div className="series-art">{art ? <img src={art} alt="" /> : <Icon icon={tv} size="2xl" />}</div>
                  ...
                  {eps.map((e) => {
                    ...
                    return (
                      <div key={eKey} className="episode">
                        <div className={`episode-row${dl ? "" : " is-missing"}`}>
